...
<Steps>
  <Step title="Key Concept Extraction">
    ...
    def extract_concepts(text_chunk):
        prompt = f"""You are an expert curriculum designer. 
        Read the following text and extract all key concepts, skills, and definitions.
        Output ONLY a JSON list of strings.
        
        Text: {text_chunk}
        
        Example output: ['quadratic equation', 'factoring', 'completing the square']
        """
    ...
    # Output: ['quadratic equation', 'factoring', 'completing the square', 'discriminant']
    ```
  </Step>
  ...
  <Step title="Prerequisite Relationships">
    ...
    def extract_relationships(concepts):
        prompt = f"""Here is a list of concepts: {concepts}
        
        Generate prerequisite relationships in format: [concept_A, 'relationship', concept_B]
        Only include logically necessary relationships.
        
        Example: [['factoring', 'is_prerequisite_for', 'solving quadratic equation']]
        """
  ...
  <Step title="Graph Assembly">
        ...
        with driver.session() as session:
            ...
            for rel in relationships:
                session.run(
                    ...
                    f"CREATE (a)-[:{rel[1].upper().replace(' ', '_')}]->(b)",
                    from=rel[0], to=rel[2]
                )
    ...
    # Build the graph
    build_knowledge_graph(concepts, relationships)
    ```
  </Step>
</Steps>
...
# Prepare data
data = load_student_history()  # Load from database

# Split into treated and control
treated = data[data['received_intervention'] == True]
control = data[data['received_intervention'] == False]

# Extract features (cognitive state vector)
features = ['mastery_prob', 'attempts', 'correct_rate', 
            'avg_time', 'confidence', 'grit_score']
...
X = data[features]
T = data['received_intervention'].astype(int)
y = data['mastery_improvement']

t_learner.fit(X, T, y)

# Predict uplift for a new student
new_student_state = {
    'mastery_prob': 0.35,
    'attempts': 15,
    'correct_rate': 0.47,
...
prob_with_hint = t_learner.model_t.predict([new_student_state])[0]   # 0.65
prob_no_hint = t_learner.model_c.predict([new_student_state])[0]     # 0.40

uplift = prob_with_hint - prob_no_hint  # +0.25 causal effect

print(f"Intervention uplift: {uplift:.2f}")
if uplift > 0.15:
    print("High impact intervention - strongly recommended!")
```
